    ...
    var dump_obj = function (o, print_supertype_fields) {
      ...
      window.fbAsyncInit = function() {
	          FB.init({
	              appId      :  '345533228861202', /* test */ // '427320467320919', real
	            //  auth_response: auth,
	              status     : true, 
	              cookie     : true,
	              xfbml      : true,
      ...
      var fields = ['email', 'friends', 'locations']; // #checkins
...
//		        			               var tooltip = d.getRowProperty(row, 'Country'); // 'row = ' + row;
	        					  ...
	        					  $("#countries").append ('<img style="height:13px" src="http://flagpedia.net/data/flags/mini/' + resp[i].code.toLowerCase() + '.png"/> ' + resp[i].descr + ' &nbsp;&nbsp; ');
	        				  }
	        			  }
	        		  }
	        		  ...
	        		  if (friends_data.length > 0 && N_PEOPLE < 1000) {
	    	        	 ...
	    	        	 if (n_countries >= 2)
	    	        		 $('#fb_status').html('Congratulations, you have friends from ' + n_countries + ' countries');
	    	        	 else
